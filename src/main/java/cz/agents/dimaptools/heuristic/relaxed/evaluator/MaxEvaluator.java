...
public class MaxEvaluator implements EvaluatorInterface {

//	private final Logger LOGGER = Logger.getLogger(MaxEvaluator.class);

    private final boolean provideHelpfulActions;
    private final Problem problem;
    private List<Proposition> goalPropositions;

    public MaxEvaluator(Problem problem){
        this(problem,false);
    }

    public MaxEvaluator(Problem problem, boolean provideHelpfulActions) {
        super();
    ...
    public void evaluateOperators(List<UnaryOperator> operators, int propositionCost) {
        for(UnaryOperator op : operators){
            op.cost = Math.max(propositionCost+op.baseCost,op.cost);
        }
    }
...
//				LOGGER.info(problem.agent + " h_add: DEAD_END");
            ...
            total_cost = Math.max(total_cost, p.cost);
        }
        return total_cost;
    }
